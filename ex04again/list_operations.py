...
def custom_remove(input_list, value):
    ...
    for i in range(custom_len(input_list)):
        if input_list[i] == value:
            break
    firsthalf = input_list[:i]
    secondhalf = input_list[i+1:]
    input_list[:i] = firsthalf
    input_list[i:] = secondhalf
...
def custom_equality(some_list, another_list):
    ...
    if custom_len(some_list) != custom_len(another_list):
        ...
        for i in range(custom_len(another_list)):
            if another_list[i] != some_list[i]:
                return False
    return True
